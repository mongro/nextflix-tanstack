import { t as smiley_default } from "./smiley-nOJo0-QZ.js";
import { t as Button } from "./button-B_qHKP16.js";
import { t as createProfile } from "./profile-DWMdx_pb.js";
import { t as Route } from "./profiles-DDbzmgB5.js";
import { t as Avatar } from "./avatar-BtgSey3w.js";
import { t as Spinner } from "./spinner-OMjEHzBU.js";
import { a as FieldGroup, i as FieldError, n as Field, o as FieldLabel, t as Input } from "./input-EPCua9JZ.js";
import { t as AvatarSelect } from "./avatar-select-rtU5fGEM.js";
import { a as DialogFooter, i as DialogDescription, o as DialogTitle, r as DialogContent, s as DialogTrigger, t as Dialog } from "./dialog-BNIhrj9v.js";
import { useActionState, useState, useTransition } from "react";
import { Link, getRouteApi, useRouter } from "@tanstack/react-router";
import { jsx, jsxs } from "react/jsx-runtime";
import z from "zod";
import { ChevronRightIcon, PencilIcon } from "@heroicons/react/24/solid";
import { zodResolver } from "@hookform/resolvers/zod";
import { Controller, useForm } from "react-hook-form";
//#region src/components/profile/create-profile-form.tsx
var createProfileFormSchema = z.object({
	name: z.string().min(1, { message: "Please enter a profile name." }),
	avatar: z.string({ message: "Select an avatar" })
});
function CreateProfileForm({ onSuccess }) {
	const router = useRouter();
	const formAction = async (prevState, formData) => {
		const result = await createProfile({ data: formData });
		if (!result.error) {
			onSuccess();
			router.invalidate();
		}
		return result;
	};
	const [actionState, submitAction, isPending] = useActionState(formAction, {
		profile: null,
		error: null
	});
	const [showAvatarSelection, setShowAvatarSelection] = useState(false);
	const [, startTransition] = useTransition();
	const form = useForm({
		resolver: zodResolver(createProfileFormSchema),
		defaultValues: {
			name: "",
			avatar: smiley_default
		}
	});
	return /* @__PURE__ */ jsx("form", {
		action: submitAction,
		onSubmit: form.handleSubmit((_, e) => {
			startTransition(() => {
				submitAction(new FormData(e?.target));
			});
		}),
		children: /* @__PURE__ */ jsxs(FieldGroup, { children: [
			/* @__PURE__ */ jsx(Controller, {
				name: "avatar",
				control: form.control,
				render: ({ field, fieldState }) => /* @__PURE__ */ jsxs(Field, {
					"data-invalid": fieldState.invalid,
					children: [/* @__PURE__ */ jsxs(Dialog, {
						open: showAvatarSelection,
						onOpenChange: setShowAvatarSelection,
						children: [/* @__PURE__ */ jsxs(DialogTrigger, {
							className: "w-1/2 max-w-64 flex items-center justify-center relative group",
							children: [/* @__PURE__ */ jsx("div", {
								className: "absolute flex items-center justify-center rounded-full size-10 group-hover:bg-background p-2 bg-background/50",
								children: /* @__PURE__ */ jsx(PencilIcon, {})
							}), /* @__PURE__ */ jsx(Avatar, {
								src: field.value,
								alt: "avatar",
								width: 512,
								height: 512
							})]
						}), /* @__PURE__ */ jsxs(DialogContent, { children: [
							/* @__PURE__ */ jsx(DialogTitle, { children: "Choose your avatar image" }),
							/* @__PURE__ */ jsx(DialogDescription, {}),
							/* @__PURE__ */ jsx(AvatarSelect, { onSelect: (avatar) => {
								form.setValue("avatar", avatar);
								console.log("set avaatr", avatar);
								setShowAvatarSelection(false);
							} })
						] })]
					}), /* @__PURE__ */ jsx(Input, {
						...field,
						id: field.name,
						"aria-invalid": fieldState.invalid,
						placeholder: "Name",
						autoComplete: "off",
						hidden: true,
						value: field.value ?? "",
						disabled: field.value === null
					})]
				})
			}),
			/* @__PURE__ */ jsx(Controller, {
				name: "name",
				control: form.control,
				render: ({ field, fieldState }) => /* @__PURE__ */ jsxs(Field, {
					"data-invalid": fieldState.invalid,
					children: [
						/* @__PURE__ */ jsx(FieldLabel, {
							htmlFor: field.name,
							children: "Name"
						}),
						/* @__PURE__ */ jsx(Input, {
							...field,
							id: field.name,
							"aria-invalid": fieldState.invalid,
							placeholder: "Name",
							autoComplete: "off"
						}),
						/* @__PURE__ */ jsxs("div", {
							className: "h-8",
							children: [actionState.error && /* @__PURE__ */ jsx(FieldError, { errors: [actionState.error] }), fieldState.invalid && /* @__PURE__ */ jsx(FieldError, { errors: [fieldState.error] })]
						})
					]
				})
			}),
			/* @__PURE__ */ jsx(DialogFooter, { children: /* @__PURE__ */ jsxs(Button, {
				type: "submit",
				disabled: isPending,
				children: [isPending && /* @__PURE__ */ jsx(Spinner, {}), isPending ? "Saving..." : "Save"]
			}) })
		] })
	});
}
//#endregion
//#region src/components/profile/create-profile-dialog.tsx
function CreateProfileDialog() {
	const [showDialog, setShowDialog] = useState(false);
	return /* @__PURE__ */ jsxs(Dialog, {
		open: showDialog,
		onOpenChange: setShowDialog,
		children: [/* @__PURE__ */ jsx(DialogTrigger, {
			asChild: true,
			children: /* @__PURE__ */ jsx(Button, {
				size: "lg",
				children: "Add profile"
			})
		}), /* @__PURE__ */ jsxs(DialogContent, { children: [/* @__PURE__ */ jsx(DialogTitle, {
			className: "text-2xl",
			children: "Add a profile"
		}), /* @__PURE__ */ jsx(CreateProfileForm, { onSuccess: () => setShowDialog(false) })] })]
	});
}
//#endregion
//#region src/components/profile/profiles.tsx
function Profiles({ profiles }) {
	const { session } = getRouteApi("/$lang").useRouteContext();
	return /* @__PURE__ */ jsx("div", {
		className: "p-4 border-2 rounded mt-4",
		children: /* @__PURE__ */ jsx("ul", { children: profiles.map((profile) => {
			return /* @__PURE__ */ jsx("li", {
				className: "border-b-2",
				children: /* @__PURE__ */ jsx(Link, {
					from: "/$lang",
					to: "/$lang/account/profiles/$id",
					params: ({ lang }) => ({
						lang,
						id: profile.id + ""
					}),
					className: "p-4 hover:bg-accent/50 block",
					children: /* @__PURE__ */ jsxs("div", {
						className: "flex items-center gap-4",
						children: [
							/* @__PURE__ */ jsx(Avatar, {
								src: profile.avatar,
								alt: "avatar",
								className: "size-12"
							}),
							/* @__PURE__ */ jsx("div", {
								className: "text-lg font-bold grow",
								children: profile.name
							}),
							profile.id === session?.session.selectedProfileId && /* @__PURE__ */ jsx("div", {
								className: "text-sm flex items-center justify-center ",
								children: "Your Profile"
							}),
							/* @__PURE__ */ jsx(ChevronRightIcon, { className: "size-6 pl-2" })
						]
					})
				})
			}, profile.name);
		}) })
	});
}
//#endregion
//#region src/routes/$lang/account/profiles/index.tsx?tsr-split=component
function ProfilesPage() {
	const profiles = Route.useLoaderData();
	return /* @__PURE__ */ jsxs("div", {
		className: "",
		children: [
			/* @__PURE__ */ jsx("h1", {
				className: "text-2xl mb-4",
				children: "Profile Settings"
			}),
			/* @__PURE__ */ jsx("div", { children: "Select the profile you want to change settings for or create a new profile." }),
			/* @__PURE__ */ jsx(Profiles, { profiles }),
			/* @__PURE__ */ jsx("div", {
				className: "mt-4",
				children: /* @__PURE__ */ jsx(CreateProfileDialog, {})
			})
		]
	});
}
//#endregion
export { ProfilesPage as component };
